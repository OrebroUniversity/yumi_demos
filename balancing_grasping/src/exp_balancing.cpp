...
ExpBalancing::ExpBalancing() {
  ...
  //  next_task_srv_ = nh_.advertiseService("next_task",
  //  &ExpBalancing::next_task_callback, this);
  quit_demo_ = nh_.advertiseService("quit_demo",
                                    &ExpBalancing::quit_demo_callback, this);

  std::string robot_namespace;
  nh_.param<std::string>("robot_namespace", robot_namespace, "yumi");
  hiqp_client_ = std::shared_ptr<hiqp_ros::HiQPClient>(new hiqp_ros::HiQPClient(
      robot_namespace, "hiqp_joint_velocity_controller"));

  tf_pub_timer = nh_.createTimer(ros::Duration(TF_PUBLISH_PERIOD),
                                 &ExpBalancing::tf_pub_callback, this);
  //  marker_pub_timer = nh_.createTimer(ros::Duration(TF_PUBLISH_PERIOD),
  //  &ExpBalancing::marker_pub_callback, this);
  quit_demo = true;
  //  next_task = false;
  ...
  // drop_assisted_task_names);
  // loadTasksFromParamServer("point_assisted_tasks", point_assisted_tasks,
  // point_assisted_task_names);
...
void ExpBalancing::expMainLoop() {
  ...
  tf::Transform L_O_T, R_O_T, target_r_frame, target_l_frame, target_obj_frame,
      current_r_frame, current_l_frame;
...
//---------------------------------------------------------------------------------------//
    ...
    hiqp_client_->waitForCompletion(pre_grasp_task_names, reactions,
                                    tolerances);
    //    cond_.notify_one();
    //---------------------------------------------------------------------------------------//
    ROS_INFO("TASK SET 3: Moving to grasp poses.");
    {
      ...
      std::list<tf::Transform> poses_l = minJerkTraj(
          current_l_frame, target_l_frame, grasp_acq_dur, TF_PUBLISH_PERIOD);
      std::list<tf::Transform> poses_obj =
          minJerkTraj(current_obj_frame, target_obj_frame, grasp_acq_dur,
                      TF_PUBLISH_PERIOD);
      prev_tasks.clear();
      ...
      while (!poses_obj.empty() && !quit_demo) {
        ...
        target_poses.push_back(
            tf::StampedTransform(poses_r.front(), ros::Time::now(),
                                 "yumi_base_link", "grasp_r_frame"));
        target_poses.push_back(
            tf::StampedTransform(poses_l.front(), ros::Time::now(),
                                 "yumi_base_link", "grasp_l_frame"));
        target_poses.push_back(
            tf::StampedTransform(poses_obj.front(), ros::Time::now(),
                                 "yumi_base_link", "obj_frame"));
        poses_r.pop_front();
        poses_l.pop_front();
        poses_obj.pop_front();
      }
    ...
    ROS_INFO("TASK SET 4: Establishing contact.");
    {
      ...
      std::list<tf::Transform> poses_r = minJerkTraj(
          current_r_frame, target_r_frame, grasp_cont_dur, TF_PUBLISH_PERIOD);
      std::list<tf::Transform> poses_l = minJerkTraj(
          current_l_frame, target_l_frame, grasp_cont_dur, TF_PUBLISH_PERIOD);
      std::list<tf::Transform> poses_obj =
          minJerkTraj(current_obj_frame, target_obj_frame, grasp_cont_dur,
                      TF_PUBLISH_PERIOD);
      ...
      while (!poses_obj.empty() && !quit_demo) {
        tf_pub_cond_.wait(lock);
        //	std::cerr<<"pushing new poses, time:
...
//---------------------------------------------------------------------------------------//
    ROS_INFO("TASK SET 5: moving to lift poses.");
    {
      ...
      std::list<tf::Transform> poses_r = minJerkTraj(
          current_r_frame, target_r_frame, grasp_lift_dur, TF_PUBLISH_PERIOD);
      std::list<tf::Transform> poses_l = minJerkTraj(
          current_l_frame, target_l_frame, grasp_lift_dur, TF_PUBLISH_PERIOD);
      std::list<tf::Transform> poses_obj =
          minJerkTraj(current_obj_frame, target_obj_frame, grasp_lift_dur,
                      TF_PUBLISH_PERIOD);
...
bool ExpBalancing::loadPreGraspPoses() {
  ...
  if (!nh_.getParam("grasping_geometric_primitives",
                    hiqp_grasping_geometric_primitives)) {
    ROS_WARN_STREAM(
        "No grasping_geometric_primitives parameter "
    ...
    for (int i = 0; i < hiqp_grasping_geometric_primitives.size(); ++i) {
      std::string frame_id = static_cast<std::string>(
          hiqp_grasping_geometric_primitives[i]["frame_id"]);
      std::string name = static_cast<std::string>(
          hiqp_grasping_geometric_primitives[i]["name"]);
      XmlRpc::XmlRpcValue &parameters_xml =
          hiqp_grasping_geometric_primitives[i]["parameters"];
      ...
      Eigen::Quaterniond q(Eigen::Matrix3d(
    ...
    for (int i = 0; i < hiqp_grasping_geometric_primitives.size(); ++i) {
      try {
        ...
        std::string type = static_cast<std::string>(
            hiqp_grasping_geometric_primitives[i]["type"]);
        std::string frame_id = static_cast<std::string>(
            hiqp_grasping_geometric_primitives[i]["frame_id"]);
        bool visible =
            static_cast<bool>(hiqp_grasping_geometric_primitives[i]["visible"]);

        XmlRpc::XmlRpcValue &color_xml =
            hiqp_grasping_geometric_primitives[i]["color"];
        XmlRpc::XmlRpcValue &parameters_xml =
            hiqp_grasping_geometric_primitives[i]["parameters"];
        ...
        hiqp_client_->setPrimitive(name, type, frame_id, visible, color,
                                   parameters);
...
bool ExpBalancing::loadTasksFromParamServer(
    std::string task_group_name, std::vector<hiqp_msgs::Task> &tasks_out,
    std::vector<std::string> &task_names_out) {

  XmlRpc::XmlRpcValue hiqp_grasping_tasks;
  bool parsing_success = true;
  if (!nh_.getParam(task_group_name, hiqp_grasping_tasks)) {
    ...
    for (int i = 0; i < hiqp_grasping_tasks.size(); ++i) {
      try {
        ...
        for (int j = 0; j < def_params_xml.size(); ++j) {
          teleop_task.def_params.push_back(
              static_cast<std::string>(def_params_xml[j]));
        }

        XmlRpc::XmlRpcValue &dyn_params_xml =
            hiqp_grasping_tasks[i]["dyn_params"];
        for (int j = 0; j < dyn_params_xml.size(); ++j) {
          teleop_task.dyn_params.push_back(
              static_cast<std::string>(dyn_params_xml[j]));
        }

        teleop_task.priority =
            static_cast<int>(hiqp_grasping_tasks[i]["priority"]);
        teleop_task.visible =
            static_cast<bool>(hiqp_grasping_tasks[i]["visible"]);
        teleop_task.active =
            static_cast<bool>(hiqp_grasping_tasks[i]["active"]);
        teleop_task.monitored =
            static_cast<bool>(hiqp_grasping_tasks[i]["monitored"]);
...
void ExpBalancing::tf_pub_callback(const ros::TimerEvent &te) {
  {
    ...
    if (lock) {
      for (unsigned int i = 0; i < target_poses.size(); i++) {
        ...
        // std::cerr<<"child_frame_id_:
        // "<<target_poses[i].child_frame_id_<<std::endl;

        // std::cerr<<"origin: "<<target_poses[i].getOrigin().getX()<<"
        // "<<target_poses[i].getOrigin().getY()<<"
        // "<<target_poses[i].getOrigin().getZ()<<std::endl<<std::endl;
        // DEBUG END =========================================
      }
...
void ExpBalancing::initializeDemo() {
  ...
  // assume that the object frame is centered in between the pre-grasp frames in
  // y, has the same x value as either pre-grasp frame and a z value of
  // pre-grasp + vertical contact lift travel
  tf::Vector3 v(pre_grasp_r_frame.getOrigin().getX(),
                (pre_grasp_l_frame.getOrigin().getY() -
                 pre_grasp_l_frame.getOrigin().getY()) /
                    2,
                pre_grasp_r_frame.getOrigin().getZ() + cont_lift);
  current_obj_frame.setOrigin(v);

  //---- setup TF frames we want to publish ----//
  //
  target_poses.push_back(tf::StampedTransform(
      pre_grasp_r_frame, ros::Time::now(), "yumi_base_link", "grasp_r_frame"));
  target_poses.push_back(tf::StampedTransform(
      pre_grasp_l_frame, ros::Time::now(), "yumi_base_link", "grasp_l_frame"));
  target_poses.push_back(tf::StampedTransform(
      current_obj_frame, ros::Time::now(), "yumi_base_link", "obj_frame"));
}

std::list<tf::Transform>
ExpBalancing::minJerkTraj(const tf::Transform &start_pose,
                          const tf::Transform &end_pose, double T, double dt) {
  ...
  Eigen::Quaterniond qi(
      start_pose.getRotation().getW(), start_pose.getRotation().getX(),
      start_pose.getRotation().getY(), start_pose.getRotation().getZ());
  Eigen::Quaterniond qe(
      end_pose.getRotation().getW(), end_pose.getRotation().getX(),
      end_pose.getRotation().getY(), end_pose.getRotation().getZ());
  double t = 0;
  for (unsigned int i = 0; i < ceil(T / dt) + 1; i++) {
    tf::Vector3 v = vi +
                    (ve - vi) * (10 * pow(t / T, 3) - 15 * pow(t / T, 4) +
                                 6 * pow(t / T, 5));
    double ip = (10 * pow(t / T, 3) - 15 * pow(t / T, 4) + 6 * pow(t / T, 5));
    Eigen::Quaterniond q = qi.slerp(ip, qe);
    poses.push_back(
        tf::Transform(tf::Quaternion(q.x(), q.y(), q.z(), q.w()), v));
    t += dt;
  }
  return poses;
}
